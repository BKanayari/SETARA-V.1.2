...
struct TreatView: View {
    ...
    var body: some View {
        VStack {
            ...
            var tmpTotal = 0
            var treatPerson = SharedPreferences.shared.getParitcipant(name: listNameTable[selectedPerson].name) ?? Participant(name: "", isParticipate: false, items: [ParticipantItem(name: "", price: 0)], total: 0)
...
struct TreatView_Previews: PreviewProvider {
    static var previews: some View {
        TreatView(listNameTable: [Participant(name: "Me", isParticipate: true, items: [], total: 100)], isPresented: .constant(true), index: 0)
    }
}
